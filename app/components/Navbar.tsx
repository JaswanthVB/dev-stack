...
export default function Navbar() {
  ...
  // Detect scroll to shrink the navbar slightly for better visibility
  useEffect(() => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] px-8 py-8 pointer-events-none">
      <div className={`max-w-7xl mx-auto flex items-center justify-between transition-all duration-500 pointer-events-auto ${scrolled ? 'translate-y-[-10px]' : ''}`}>
        ...
        {/* LOGO: Vault.sys */}
        <Link href="/" className="mix-blend-lighten group">
        ...
        {/* NAVIGATION HUB: The Command Center */}
        <div className="flex items-center gap-1 bg-[#161F2E]/80 backdrop-blur-2xl border border-white/10 p-1.5 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          
          {/* Home Button: Only visible when not on the landing page */}
          {!isHomePage && (
            <Link 
              href="/" 
          ...
          {/* THE MAIN ACTION: Initiate Handshake */}
 ...
 * Includes an "Active" state that glows when you are on that page
 */
